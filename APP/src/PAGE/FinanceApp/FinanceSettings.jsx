...
const FinanceSettings = ({ user, onUserUpdate, onLogout }) => {
  ...
  const forceLogoutAfterChange = () => {
    ...
    setTimeout(() => {
      ...
      window.location.href = '/app/finance/login';
    }, 1500);
  };
  ...
  const handleChangeUsername = async (e) => {
    ...
    try {
      const response = await api.post('/user-profile/staff/change-username', {
  ...
  const handleChangePassword = async (e) => {
    ...
    try {
      const response = await api.post('/user-profile/staff/change-password', {
  ...
  return (
    <div className={styles.settingsContainer}>
      ...
      <p className={styles.settingsSubtitle}>
        {t('financeApp.shell.settings.loggedInAs', { username: user?.username, branchCode: user?.branchCode })}
        {' — '}
